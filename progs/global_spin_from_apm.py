...
with open("../logs/spin_fit.txt", "w") as f_out:
    ...
    print("num_sou, rx_mean, ry_mean, rz_mean, r_mean, "
          "rx_std, ry_std, rz_std, r_std, "
          "rx_q1, ry_q1, rz_q1, r_q1, "
          "rx_q2, ry_q2, rz_q2, r_q2, "
          "rx_q3, ry_q3, rz_q3, r_q3", file=f_out)
    ...
    for i, num_soui in enumerate(num_sou):
        ...
        print("Iteration : [{:4d}/{:4d}]".format(i, len(num_sou)))

        pmt = np.zeros((num_iter, 4))
        sig = np.zeros((num_iter, 4))
        ...
        for j in range(num_iter):
            ...
            # Do the LSQ fitting
            pmt[j, :], sig[j, :] = vsh_fit_for_pm(new_tab)
